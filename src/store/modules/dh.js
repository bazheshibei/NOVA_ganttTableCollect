    ...
    A_adjustmentNodeTest({ state }, { that, item_id, item_node_id }) {
      ...
      const suc = function (res) {
        ...
        if (String(status) === '0') {
          ...
          const url = window.location.origin + `/nova/itemNodeAdjustmentShowAction.do?action=showAdd&item_id=${item_id}&item_node_id=${item_node_id}&adjustment_type=2&item_gantt_id=${item_gantt_id}&item_gantt_detail_id=${item_gantt_detail_id}&gantt_type=1`
          const param = { item_id, item_node_id, adjustment_type: 2, item_gantt_id, item_gantt_detail_id, gantt_type: 1 }
          // eslint-disable-next-line
