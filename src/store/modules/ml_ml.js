...
const MlMl = {
  ...
  actions: {
     ...
     */
    A_tableData({ state }) {
      // const res = LocalData['面料：汇总列表']
      // console.log('面料__面料', res.data)
      ...
      if (!loadingPage) {
        ...
        /* 发起请求 */
        const name = '统计列表'
        const obj = { filter_data: JSON.stringify(filter_data), titleSearch: JSON.stringify(tableHeader), postEmpSearch: JSON.stringify(businessObj), type: 5, page: parseInt(pagenum) - 1, num: rownum, empid }
    ...
    A_tableOtherData({ state }, { row }) {
      // const res = LocalData['面料：汇总折叠']
      // console.log('面料__面料__折叠', res.data)
      // const { index } = row
      // const { data, nums } = res
      ...
      const suc = function (res) {
        // localStorage.setItem('面料__面料__折叠', JSON.stringify(res))
        const { data, nums } = res
        const { tableData_2, loading } = state
        /* 给数据添加属性 */
        const { yjts } = state
    ...
    A_goBackAudit({ state }, { params, that }) {
      ...
      Api({ name, obj, suc, loading: '撤销审核中...' })
    },
    /**
     * [请求：节点变更前验证]
     */
    A_beforBatchAdjusmentItemGantt({ state }, { item_gantt_id, item_id, that }) {
      ...
      //   /* 保存到本地缓存 */
      //   const obj = data[0]
      //   localStorage.setItem('NOVA_ItemNodeAdjustmentHtml', JSON.stringify(Object.assign({}, obj, { page_type: 'add', item_id, item_gantt_id, gantt_type: 5 })))
      //   /* win 方法打开页面 */
    ...
    A_adjustmentNodeTest({ state }, { that, item_id, item_node_id }) {
      ...
      const suc = function (res) {
        ...
        if (String(status) === '0') {
          ...
          const url = window.location.origin + `/nova/itemNodeAdjustmentShowAction.do?action=showAdd&item_id=${item_id}&item_node_id=${item_node_id}&adjustment_type=2&item_gantt_id=${item_gantt_id}&item_gantt_detail_id=${item_gantt_detail_id}&gantt_type=5`
          const param = { item_id, item_node_id, adjustment_type: 2, item_gantt_id, item_gantt_detail_id, gantt_type: 5 }
          // eslint-disable-next-line
