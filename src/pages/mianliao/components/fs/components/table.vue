...
<template>
  ...
  <div>
    ...
    >
      ...
      <!-- 面料颜色 -->
      <el-table-column prop="material_color_name" min-width="120">
        <template slot="header" slot-scope="scope">
          <el-popover placement="top" width="250" trigger="click">
            <el-input ref="input_5" v-model="tableHeader.material_color_name" clearable  size="mini" placeholder="多个查询空格分隔" @clear="clear('material_color_name')" @change="select"></el-input>
            <div class="thText" slot="reference" style="flex: 1;" @click="tableHeaderClick('input_5')">
              面料颜色<span>&nbsp;<i class="el-icon-search" :class="tableHeader.material_color_name ? 'thActive' : ''"></i></span>
            </div>
          </el-popover>
        </template>
      </el-table-column>
      <!-- 开发工厂 -->
      <el-table-column prop="material_provider_name" min-width="120">
        <template slot="header" slot-scope="scope">
          <el-popover placement="top" width="250" trigger="click">
            <el-input ref="input_6" v-model="tableHeader.material_provider_name" clearable  size="mini" placeholder="多个查询空格分隔" @clear="clear('material_provider_name')" @change="select"></el-input>
            <div class="thText" slot="reference" style="flex: 1;" @click="tableHeaderClick('input_6')">
              开发工厂<span>&nbsp;<i class="el-icon-search" :class="tableHeader.material_provider_name ? 'thActive' : ''"></i></span>
            </div>
          </el-popover>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="matter_release_time" min-width="100">
        <template slot="header" slot-scope="scope">
          <el-popover placement="top" width="250" trigger="click">
            <el-input ref="input_10" v-model="tableHeader.matter_release_time" clearable  size="mini" placeholder="多个查询空格分隔" @clear="clear('matter_release_time')" @change="select"></el-input>
...
export default {
  ...
  computed: {
    ...mapState('MlFs', ['tableData_1', 'tableData_2', 'tableNodes', 'pageObj', 'loading', 'businesspost'])
  },
  methods: {
    /**
     * [表头查询：点击表头，input自动聚焦]
    ...
    select() {
      ...
      /* 保存表头数据 */
      this.$store.commit('saveData', { module: 'MlFs', name: 'tableHeader', obj: obj_1 })
      this.$store.commit('saveData', { module: 'MlFs', name: 'businessObj', obj: obj_2 })
      /** 查询 **/
      this.$store.dispatch('MlFs/A_tableData')
    },
    /**
    ...
    rowClick(row, column, event) {
      ...
      this.$store.commit('saveData', { name: 'item_id', obj: item_id, module: 'MlFs' })
      this.$store.commit('saveData', { name: 'item_gantt_id', obj: item_gantt_id, module: 'MlFs' })
      this.$store.commit('saveData', { name: 'item_gantt_detail_id', obj: item_gantt_detail_id, module: 'MlFs' })
      this.$store.commit('saveData', { name: 'isChangeNodes', obj: audit_status === '提报:审核通过', module: 'MlFs' })
      this.$store.commit('saveData', { name: 'isEdit', obj: isEdit, module: 'MlFs' })
      this.$store.commit('saveData', { name: 'plant_order_id', obj: plant_order_id, module: 'MlFs' })
      this.$store.commit('saveData', { name: 'tableRow_1', obj: row, module: 'MlFs' })
      /* 展示折叠内容 */
      this.showMore(row)
    },
    /**
