      ...
      <div>
        ...
        <el-dropdown size="mini" trigger="click" @command="dropdown">
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown-item command="history" :disabled="!item_id">历史变更记录</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
        <el-button type="primary" size="mini" :disabled="!item_id" @click="look">制定/查看面料分色计划</el-button>
      </div>
      <div>
...
export default {
  ...
  created() {
    ...
    this._countHeight()
    /** 请求：甘特表帮助按钮 **/
    this.$store.dispatch('MlMl/A_getHelpText')
  },
  computed: {
    ...mapState('MlMl', ['pagenum', 'rownum', 'pageCount', 'item_id', 'item_gantt_id', 'item_gantt_detail_id', 'disabledChange', 'isChangeNodes', 'isEdit', 'helpText', 'tableRow_1', 'plant_order_id'])
  },
  methods: {
    /**
     * [查看]
    ...
    nodeChange() {
      ...
      /** 请求：节点变更前验证 **/
      this.$store.dispatch('MlMl/A_beforBatchAdjusmentItemGantt', { item_gantt_id, item_id, that: this })
    },
    /**
     * [历史变更记录]
     */
    ...
    beforeSubmit(audit_status) {
      this.$store.dispatch('MlMl/A_beforeSubmitAudit', { audit_status, that: this })
    },
    /**
     * [审核操作前的验证：返回值]
    ...
    submitApi(params, audit_status) {
      if (audit_status === 1) {
        this.$store.dispatch('MlMl/A_submitAudit', { params, that: this })
      } else if (audit_status === 2) {
        this.$store.dispatch('MlMl/A_goBackAudit', { params, that: this })
      }
      /* 关闭弹出层 */
    ...
    look() {
      ...
      // eslint-disable-next-line
      updateWin({ title: '制定面料分色计划', width: 1800, height: 900, url, param: { itemids: item_id }, onClose: function () {}, fn: function () { that.f5(false) } })
    },
    /**
     * [高级查询]
     */
    advancedQuery() {
      this.$store.commit('saveData', { name: 'isDialog', obj: true, module: 'MlMl' })
    },
    /**
     * [刷新]
    ...
    f5(reset = true) {
      ...
      this.$store.commit('saveData', { module: 'MlMl', name: 'item_id', obj: '' })
      this.$store.commit('saveData', { module: 'MlMl', name: 'item_gantt_id', obj: '' })
      this.$store.commit('saveData', { module: 'MlMl', name: 'item_gantt_detail_id', obj: '' })
      /** 发起请求 **/
      this._request()
    },
    /**
    ...
    pageChange(name, val) {
      /** 保存数据 **/
      this.$store.commit('saveData', { module: 'MlMl', name, obj: val })
      if (name === 'rownum') {
        this.$store.commit('saveData', { module: 'MlMl', name: 'pagenum', obj: 1 })
      }
      /** 发起请求 **/
