...
<template>
  <div>
    ...
    >
      ...
      <!-- 物料名称 -->
      <el-table-column prop="material_name" width="220">
        <template slot="header" slot-scope="scope">
          <el-popover placement="top" width="250" trigger="click">
            <el-input ref="input_6" v-model="tableHeader.material_name" clearable  size="mini" placeholder="多个查询空格分隔" @clear="clear('material_name')" @change="select"></el-input>
            <div class="thText" slot="reference" style="flex: 1;" @click="tableHeaderClick('input_6')">
              物料名称<span>&nbsp;<i class="el-icon-search" :class="tableHeader.material_name ? 'thActive' : ''"></i></span>
            </div>
          </el-popover>
        </template>
      </el-table-column>
      <!-- 物料颜色 -->
      <el-table-column prop="color_name" width="120">
        <template slot="header" slot-scope="scope">
          <el-popover placement="top" width="250" trigger="click">
            <el-input ref="input_7" v-model="tableHeader.color_name" clearable  size="mini" placeholder="多个查询空格分隔" @clear="clear('color_name')" @change="select"></el-input>
            <div class="thText" slot="reference" style="flex: 1;" @click="tableHeaderClick('input_7')">
              物料颜色<span>&nbsp;<i class="el-icon-search" :class="tableHeader.color_name ? 'thActive' : ''"></i></span>
            </div>
          </el-popover>
        </template>
      </el-table-column>
...
export default {
  ...
  computed: {
    ...mapState('DhMl', ['tableData_1', 'tableData_2', 'tableNodes', 'pageObj', 'loading', 'businesspost'])
  },
  methods: {
    /**
     * [表头查询：点击表头，input自动聚焦]
    ...
    select() {
      ...
      /* 保存表头数据 */
      this.$store.commit('saveData', { module: 'DhMl', name: 'tableHeader', obj: obj_1 })
      this.$store.commit('saveData', { module: 'DhMl', name: 'businessObj', obj: obj_2 })
      /** 查询 **/
      this.$store.dispatch('DhMl/A_tableData')
    },
    /**
