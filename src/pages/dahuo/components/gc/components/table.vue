...
export default {
  ...
  methods: {
    ...
    rowClick(row, column, event) {
      ...
      this.$store.commit('saveData', { name: 'item_id', obj: item_id, module: 'DhGc' })
      this.$store.commit('saveData', { name: 'item_gantt_id', obj: item_gantt_id, module: 'DhGc' })
      this.$store.commit('saveData', { name: 'item_gantt_detail_id', obj: item_gantt_detail_id, module: 'DhGc' })
      this.$store.commit('saveData', { name: 'isChangeNodes', obj: audit_status === '提报:审核通过', module: 'DhGc' })
      this.$store.commit('saveData', { name: 'isEdit', obj: isEdit, module: 'DhGc' })
      this.$store.commit('saveData', { name: 'plant_order_id', obj: plant_order_id, module: 'DhGc' })
      this.$store.commit('saveData', { name: 'tableRow_1', obj: row, module: 'DhGc' })
      /* 展示折叠内容 */
      this.showMore(row)
    },
    /**
    ...
    showMore(row) {
      ...
      if (!tableData_2[index]) {
        loading[index] = true
        /** 请求：表格折叠数据 **/
        this.$store.dispatch('DhGc/A_tableOtherData', { row })
      }
      /* 显示 || 隐藏 */
