...
export default {
  ...
  methods: {
    ...
    _proving() {
      ...
      /* 保存有用数据 */
      this.$store.commit('saveData', { name: 'filter_data', obj: arr, module: 'DhGc' })
    },
    /**
     * [增加条件]
     */
    ...
    reset() {
      ...
      this.$store.commit('saveData', { name: 'filter_data', obj: [], module: 'DhGc' })
      /* 重置分页 */
     ...
     */
    handleClose() {
      this.$store.commit('saveData', { name: 'isDialog', obj: false, module: 'DhGc' })
    },
    /**
     * [计算：表格高度]
     */
