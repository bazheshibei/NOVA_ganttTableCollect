...
export default {
  ...
  methods: {
    ...
    complete() {
      ...
      /** 请求：节点完成前验证 **/
      this.$store.dispatch('DhGc/A_testItemNodeStatus', { item_id, item_node_id, completion_method, index, that: this })
    },
    /**
     * [节点跟进]
     */
    ...
    change() {
      ...
      /** 请求：单独变更节点前验证 **/
      this.$store.dispatch('DhGc/A_adjustmentNodeTest', { that, item_id, item_node_id })
    },
    /**
     * [取消完成]
     * @click {[choiceRow.item_node_id]}    选中节点
    ...
    clickSearch() {
      ...
      this.$store.commit('saveData', { name: 'node_name', obj: node_name, module: 'DhGc' })
      this.$store.commit('saveData', { name: 'status', obj: status, module: 'DhGc' })
      /** 刷新 **/
      this.f5(true)
    },
    /**
    ...
    clickReset() {
      this.node_name = ''
      this.status = ''
      this.$store.commit('saveData', { name: 'node_name', obj: '', module: 'DhGc' })
      this.$store.commit('saveData', { name: 'status', obj: '', module: 'DhGc' })
      /** 刷新 **/
      this.f5(true)
    },
    /**
    ...
    f5(reset = true) {
      if (reset) {
        ...
        this.$store.commit('assignData', { name: 'pageObj', obj: pageObj, module: 'DhGc' })
      }
      this.choiceRow = {}
      /** 发起请求 **/
      this._request()
