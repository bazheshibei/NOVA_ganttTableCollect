...
export default {
  ...
  created() {
    ...
    this._countHeight()
    /** 请求：甘特表帮助按钮 **/
    this.$store.dispatch('DhGc/A_getHelpText')
  },
  computed: {
    ...mapState('DhGc', ['pagenum', 'rownum', 'pageCount', 'item_id', 'item_gantt_id', 'item_gantt_detail_id', 'disabledChange', 'isChangeNodes', 'isEdit', 'helpText', 'tableRow_1', 'plant_order_id'])
  },
  methods: {
    /**
     * [编辑]
    ...
    edit() {
      const that = this
      /* 保存到本地缓存 */
      const { item_id, plant_order_id } = this
      localStorage.setItem('NOVA_orderItemNodeFrom', JSON.stringify({ item_id, plant_order_id }))
      /* win 方法打开页面 */
      const url = window.location.origin + '/nova/pages/plantorder/orderItemNodeFrom.html'
    ...
    nodeChange() {
      ...
      /** 请求：节点变更前验证 **/
      this.$store.dispatch('DhGc/A_beforBatchAdjusmentItemGantt', { item_gantt_id, item_id, that: this })
    },
    /**
     * [历史变更记录]
     */
    ...
    beforeSubmit(audit_status) {
      this.$store.dispatch('DhGc/A_beforeSubmitAudit', { audit_status, that: this })
    },
    /**
     * [审核操作前的验证：返回值]
    ...
    submitApi(params, audit_status) {
      if (audit_status === 1) {
        this.$store.dispatch('DhGc/A_submitAudit', { params, that: this })
      } else if (audit_status === 2) {
        this.$store.dispatch('DhGc/A_goBackAudit', { params, that: this })
      }
      /* 关闭弹出层 */
     ...
     */
    advancedQuery() {
      this.$store.commit('saveData', { name: 'isDialog', obj: true, module: 'DhGc' })
    },
    /**
     * [刷新]
    ...
    f5(reset = true) {
      ...
      this.$store.commit('saveData', { module: 'DhGc', name: 'item_id', obj: '' })
      this.$store.commit('saveData', { module: 'DhGc', name: 'item_gantt_id', obj: '' })
      this.$store.commit('saveData', { module: 'DhGc', name: 'item_gantt_detail_id', obj: '' })
      /** 发起请求 **/
      this._request()
    },
    /**
    ...
    pageChange(name, val) {
      /** 保存数据 **/
      this.$store.commit('saveData', { module: 'DhGc', name, obj: val })
      if (name === 'rownum') {
        this.$store.commit('saveData', { module: 'DhGc', name: 'pagenum', obj: 1 })
      }
      /** 发起请求 **/
