...
<template>
  <div class="comBox" ref="page">
    ...
    <!-- 弹出层：编辑 -->
    <el-dialog title="请选择要编辑的项" :visible.sync="dialogVisible_edit" width="40%" :close-on-click-modal="false" :close-on-press-escape="false">
...
export default {
  ...
  created() {
    ...
    this._countHeight()
    /** 请求：甘特表帮助按钮 **/
    this.$store.dispatch('DhDh/A_getHelpText')
  },
  computed: {
    ...mapState('DhDh', ['pagenum', 'rownum', 'pageCount', 'item_id', 'item_gantt_id', 'item_gantt_detail_id', 'disabledChange', 'isChangeNodes', 'isEdit', 'helpText', 'tableRow_1'])
  },
  methods: {
    /**
     * [编辑]
    ...
    nodeChange() {
      ...
      /** 请求：节点变更前验证 **/
      this.$store.dispatch('DhDh/A_beforBatchAdjusmentItemGantt', { item_gantt_id, item_id, that: this })
    },
    /**
     * [历史变更记录]
     */
    ...
    beforeSubmit(audit_status) {
      this.$store.dispatch('DhDh/A_beforeSubmitAudit', { audit_status, that: this })
    },
    /**
     * [审核操作前的验证：返回值]
    ...
    submitApi(params, audit_status) {
      if (audit_status === 1) {
        this.$store.dispatch('DhDh/A_submitAudit', { params, that: this })
      } else if (audit_status === 2) {
        this.$store.dispatch('DhDh/A_goBackAudit', { params, that: this })
      }
      /* 关闭弹出层 */
     ...
     */
    advancedQuery() {
      this.$store.commit('saveData', { name: 'isDialog', obj: true, module: 'DhDh' })
    },
    /**
     * [刷新]
    ...
    f5(reset = true) {
      ...
      console.log('刷新')
      this.$store.commit('saveData', { module: 'DhDh', name: 'item_id', obj: '' })
      this.$store.commit('saveData', { module: 'DhDh', name: 'item_gantt_id', obj: '' })
      this.$store.commit('saveData', { module: 'DhDh', name: 'item_gantt_detail_id', obj: '' })
      /** 发起请求 **/
      this._request()
    },
    /**
    ...
    pageChange(name, val) {
      /** 保存数据 **/
      this.$store.commit('saveData', { module: 'DhDh', name, obj: val })
      if (name === 'rownum') {
        this.$store.commit('saveData', { module: 'DhDh', name: 'pagenum', obj: 1 })
      }
      /** 发起请求 **/
