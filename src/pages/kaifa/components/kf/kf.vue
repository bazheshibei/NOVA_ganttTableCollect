...
<template>
  <div class="comBox" ref="page">
      ...
      <div>
        ...
        <el-dropdown size="mini" trigger="click" @command="dropdown">
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown-item command="history" :disabled="!item_gantt_id">历史变更记录</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </div>
      <div>
    ...
    <!-- 弹出层：提交/撤销选节点 || 填写撤销说明 -->
    <el-dialog :visible.sync="dialogVisible_submit" width="40%" :close-on-click-modal="false" :close-on-press-escape="false">
...
export default {
  ...
  created() {
    ...
    this._countHeight()
    /** 请求：甘特表帮助按钮 **/
    this.$store.dispatch('KfKf/A_getHelpText')
  },
  computed: {
    ...mapState('KfKf', ['pagenum', 'rownum', 'pageCount', 'item_id', 'item_gantt_id', 'item_gantt_detail_id', 'disabledChange', 'isChangeNodes', 'isEdit', 'helpText', 'tableRow_1'])
  },
  methods: {
    /**
     * [编辑]
    ...
    nodeChange() {
      ...
      /** 请求：节点变更前验证 **/
      this.$store.dispatch('KfKf/A_beforBatchAdjusmentItemGantt', { item_gantt_id, item_id, that: this })
    },
    /**
     * [历史变更记录]
     */
    history() {
      ...
      ui("open", {
        title: '历史变更记录',
        url: `${host}pages/itemnodeadjustment/itemNodeAdjustmentlist.jsp?item_id=${item_id}`,
        onClose: function () {}
      })
    },
    /**
    ...
    beforeSubmit(audit_status) {
      this.$store.dispatch('KfKf/A_beforeSubmitAudit', { audit_status, that: this })
    },
    /**
     * [审核操作前的验证：返回值]
    ...
    submitApi(params, audit_status) {
      if (audit_status === 1) {
        this.$store.dispatch('KfKf/A_submitAudit', { params, that: this })
      } else if (audit_status === 2) {
        this.$store.dispatch('KfKf/A_goBackAudit', { params, that: this })
      }
      /* 关闭弹出层 */
    ...
    dialogChange() {
      ...
      // eslint-disable-next-line
      win({ title: '批量变更节点', width: 1500, height: 600, url, param: {}, fn() { that.f5(false) }, onClose() { that.f5(false) } })
      /* 关闭弹出层 */
     ...
     */
    advancedQuery() {
      this.$store.commit('saveData', { name: 'isDialog', obj: true, module: 'KfKf' })
    },
    /**
     * [刷新]
    ...
    f5(reset = true) {
      ...
      this.$store.commit('saveData', { module: 'KfKf', name: 'item_id', obj: '' })
      this.$store.commit('saveData', { module: 'KfKf', name: 'item_gantt_id', obj: '' })
      this.$store.commit('saveData', { module: 'KfKf', name: 'item_gantt_detail_id', obj: '' })
      /** 发起请求 **/
      this._request()
    },
    /**
    ...
    pageChange(name, val) {
      /** 保存数据 **/
      this.$store.commit('saveData', { module: 'KfKf', name, obj: val })
      if (name === 'rownum') {
        this.$store.commit('saveData', { module: 'KfKf', name: 'pagenum', obj: 1 })
      }
      /** 发起请求 **/
