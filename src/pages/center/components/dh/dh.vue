...
export default {
  ...
  methods: {
    ...
    beforeSubmit(audit_status) {
      this.$store.dispatch('Dh/A_beforeSubmitAudit', { audit_status, that: this })
    },
    /**
     * [审核操作前的验证：返回值]
    ...
    submitApi(params, audit_status) {
      if (audit_status === 1) {
        this.$store.dispatch('Dh/A_submitAudit', { params, that: this })
      } else if (audit_status === 2) {
        this.$store.dispatch('Dh/A_goBackAudit', { params, that: this })
      }
      /* 关闭弹出层 */
